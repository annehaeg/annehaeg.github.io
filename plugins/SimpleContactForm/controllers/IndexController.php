...
class SimpleContactForm_IndexController extends Omeka_Controller_AbstractActionController
{
    ...
    protected function _validateFormSubmission($captcha = null)
    {
        ...
        if ($captcha and !$captcha->isValid('foo', $_POST)) {
            $this->_helper->flashMessenger(__('De CAPTCHA-invoer was incorrect, gelieve opnieuw te proberen.'));
            $valid = false;
        } else if (!Zend_Validate::is($email, 'EmailAddress')) {
            $this->_helper->flashMessenger(__('Geef een geldig emailadres op.'));
            $valid = false;
        } else if (empty($msg)) {
            $this->_helper->flashMessenger(__('Geef je boodschap op..'));
            $valid = false;
        }
